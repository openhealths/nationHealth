...
class CompositionTempDisabilityFormTest extends TestCase
{
    ...
    public function test_a_period_that_ends_before_it_starts_is_rejected(): void
    {
        $validator = $this->validate([
            'eventPeriodStart' => '10.08.2026',
            'eventPeriodEnd' => '01.08.2026',
        ]);
    ...
    {
        $validator = $this->validate([
            'eventPeriodStart' => '05.08.2026',
            'eventPeriodEnd' => '20.08.2026',
            'treatmentViolation' => 'reject_recommendation',
            'treatmentViolationDate' => '01.08.2026',
        ]);
    ...
    public function test_a_violation_inside_the_period_is_accepted(): void
    {
        $validator = $this->validate([
            ...
            'eventPeriodEnd' => '20.08.2026',
            'treatmentViolation' => 'reject_recommendation',
            'treatmentViolationDate' => '07.08.2026',
        ]);

        $this->assertFalse($validator->fails(), $validator->errors()->toJson());
    }

    public function test_a_violation_date_in_the_future_is_rejected(): void
    {
        $validator = $this->validate([
            'eventPeriodStart' => now()->format(config('app.date_format')),
            'eventPeriodEnd' => now()->addDays(7)->format(config('app.date_format')),
            'treatmentViolation' => 'reject_recommendation',
            'treatmentViolationDate' => now()->addDay()->format(config('app.date_format')),
        ]);
    ...
    public function test_prefill_reads_the_period_and_flags_of_the_previous_conclusion(): void
    {
        ...
        $this->assertSame('01.07.2026', $form->eventPeriodStart);
        $this->assertSame('14.07.2026', $form->eventPeriodEnd);
        $this->assertTrue($form->isAccident);
        $this->assertFalse($form->isIntoxicated);
        $this->assertSame('hospital_leave', $form->treatmentViolation);
        $this->assertSame('05.07.2026', $form->treatmentViolationDate);
        $this->assertSame(
    ...
    public function test_mapper_data_carries_every_field_the_payload_needs(): void
    {
        ...
        $form->eventPeriodStart = '01.08.2026';
        $form->eventPeriodEnd = '10.08.2026';
    ...
    private function validate(array $overrides = []): ValidatorContract
    {
        ...
        $data = array_merge([
            ...
            'eventPeriodStart' => '01.08.2026',
            'eventPeriodEnd' => '10.08.2026',
            'isAccident' => false,
            'isIntoxicated' => false,
