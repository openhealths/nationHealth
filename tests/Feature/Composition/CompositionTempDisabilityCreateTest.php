...
class CompositionTempDisabilityCreateTest extends TestCase
{
    ...
    public function test_the_details_and_review_steps_render(): void
    {
        ...
        $component->set('compositionDetail', [
            ...
            'extension' => [
                ...
                ['valueCode' => 'TREATMENT_VIOLATION', 'valueString' => 'reject_hospitalization'],
                ['valueCode' => 'TREATMENT_VIOLATION_DATE', 'valueDate' => '2026-08-05'],
            ],
        ])
    ...
    public function test_the_review_step_after_signing_shows_signed_copy_without_sign_button(): void
    {
        ['legalEntity' => $legalEntity] = $this->fixture();
        $this->fakeEncounters();
    ...
    public function test_opening_with_an_encounter_query_skips_the_picker(): void
    {
        ['legalEntity' => $legalEntity, 'employee' => $employee] = $this->fixture();
        ...
        Livewire::withQueryParams(['encounter' => $encounterUuid])
            ->test(CompositionTempDisabilityCreate::class, [
                'legalEntity' => $legalEntity,
                'preperson' => $this->preperson(),
            ])
            ->assertSet('encounter', $encounterUuid)
            ->assertSet('form.encounterUuid', $encounterUuid)
            ->assertSet('episodeUuid', $episodeUuid)
            ->assertSet('step', CompositionTempDisabilityCreate::STEP_AUTH_METHOD);
    }
