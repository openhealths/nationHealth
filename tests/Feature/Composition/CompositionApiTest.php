...
class CompositionApiTest extends TestCase
{
    ...
    public function test_create_posts_to_the_collection_endpoint_without_a_patient_in_the_path(): void
    {
        ...
        $this->makeApi()->create(['type' => ['coding' => [['system' => 'COMPOSITION_TYPES', 'code' => 'NEWBORN']]]]);
    ...
    public function test_create_sends_the_composition_request_body_not_a_signature_wrapper(): void
    {
        ...
        $this->makeApi()->create($payload);

        Http::assertSent(static function (Request $request) use ($payload): bool {
    ...
    /**
     * Sign/cancel still carry PKCS#7 under `data`; create does not — so redaction of
     * `data` remains required for those endpoints.
     */
