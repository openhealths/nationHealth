...
class CompositionCreateFlowTest extends TestCase
{
    ...
    /**
     * Nothing may reach eHealth before the author has signed: the details step only
     * opens the KEP dialog.
     */
    public function test_reviewing_the_details_opens_the_signature_dialog_without_creating_anything(): void
    {
        ...
        Http::assertNotSent(static fn ($request): bool => str_ends_with($request->url(), '/composition'));
    }

    /**
     * The wizard binds the details-step signature dialog to this action, so its absence
     * is what made the whole create path unreachable.
     */
    public function test_the_wizard_exposes_the_action_its_signature_dialog_calls(): void
    {
        $this->assertTrue(
            method_exists(CompositionTempDisabilityCreate::class, 'submitComposition'),
            'The details-step signature dialog calls submitComposition.'
        );
    }

    public function test_submitting_sends_the_signed_payload_and_starts_polling_the_job(): void
    {
        ...
        Http::assertSent(static fn ($request): bool => str_ends_with($request->url(), '/composition')
            && $request->data() === ['data' => 'signed-composition']);
    }

    public function test_submitting_without_signing_credentials_is_refused(): void
    {
        $this->fakeSignatureService();
        ...
        $component->call('submitComposition')
            ->assertHasErrors(['form.knedp', 'form.password', 'form.keyContainerUpload'])
    ...
    /**
     * The end-to-end lifecycle: signed create, job polling, read-back, sign, and the
