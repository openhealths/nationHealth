...
class CompositionMapperTest extends TestCase
{
    ...
    public function a_birth_conclusion_has_an_open_ended_period_starting_on_the_birth_date(): void
    {
        ...
        $this->assertSame('2026-08-13T00:00:01Z', $payload['event'][0]['period']['start']);
        $this->assertArrayHasKey('end', $payload['event'][0]['period']);
        $this->assertNull(
            $payload['event'][0]['period']['end'],
            'Open-ended МВН period must send end: null, matching Confluence createComposition examples.'
        );
        $this->assertArrayNotHasKey(
            'text',
            $payload['type'],
            'type must match Confluence examples: coding only, no empty text.'
        );
