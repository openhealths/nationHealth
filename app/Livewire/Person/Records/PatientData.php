...
class PatientData extends BasePatientComponent
{
    ...
    public function syncPersonDataFromEHealth(): void
    {
        ...
        $requestData = MERepository::approval()->formatEHealthRequest($payloadData);

        $approval = Approval::where('status', Status::NEW->value)
            ->where('approvable_type', Person::class)
            ->where('approvable_id', $this->personId)
            ->first() ?? MERepository::approval()->create($requestData, $person);
        ...
        try {
            ...
            $exception->handle(__('Error throughout creating approval for getting a person data'));
        ...
        if (empty($links) && isset($responseData['is_verified'])) {
            foreach ($responseData['granted_resources'] ?? [] as $grantedResource) {
                ...
                MERepository::approval()->sync(modelData: $responseData, approvableModel: $person, approvalModel: $approval);
            }
