...
- **FR-022**: Рівень доступу `write` MUST вимагатися для створення/зміни activities і виписки документів іншим клініцистом того ж СГуСОЗ без implicit access.
- **FR-023**: МІС MUST ініціативно пояснювати `403 Access denied` як проблему згоди, з CTA на Approval.
- **FR-024**: Наявність активної декларації MAY замінювати явний Approval для лікаря декларації (implicit access); МІС MUST коректно визначати цей випадок.
