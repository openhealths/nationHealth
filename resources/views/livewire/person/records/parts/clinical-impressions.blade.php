...
<div @if($hasLimit) x-data="{ limit: {{ $limit }} }" @endif>
        ...
        <div class="record-inner-card" @if($hasLimit) x-show="limit > {{ $index }}" @endif>
            ...
            <div class="record-inner-body">
                <div class="record-inner-grid-container">
                    <div
                        class="grid grid-cols-2 xl:grid-cols-5 gap-y-4 gap-x-4 w-full [&>div]:min-w-0 [&_.record-inner-value]:break-words">
                        <div>
                            <div class="record-inner-label">{{ __('patients.created') }}</div>
                            <div
                                class="record-inner-value">{{ data_get($clinicalImpression, 'ehealthInsertedAt', '-') }}</div>
                        </div>
                        <div>
    ...
    @if($hasLimit)
        <div x-show="limit < {{ count($this->clinicalImpressions) }}" class="flex justify-start mt-4">
            <button type="button"
                    @click="limit += 5"
                    class="item-add"
            >
