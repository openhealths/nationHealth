...
>
    <x-slot name="title">
        {{ __('patients.merge.electronic_signature') }}
    </x-slot>
        ...
        <div class="space-y-6 max-w-2xl">
         <div>
             ...
             <select class="input-modal w-full" x-model="selectedKnedp" name="mergeKnedp" id="mergeKnedp">
                 <option value="" selected>{{ __('patients.merge.select_knedp') }}</option>
         ...
         <div>
             <label class="default-label">{{ __('patients.merge.container_path') }}</label>
             ...
             >
                 <div class="flex flex-col items-center justify-center pt-5 pb-6">
                     ...
                     <p class="mb-2 px-2 text-sm text-gray-500 dark:text-gray-400 text-center">
                         <span class="font-semibold text-blue-600 dark:text-blue-400">{{ __('patients.merge.drag_key_file') }}</span>
                         {{ __('patients.merge.or_upload_from_device') }}
                     </p>
                     <p class="px-2 text-xs text-gray-500 dark:text-gray-400 text-center">
                         {{ __('patients.merge.key_file_extension_hint') }}
                     </p>
                 </div>
             ...
             <template x-if="fileUploaded">
                 <div x-transition class="text-sm text-green-700 mt-2 font-medium">
                     {!! __('patients.merge.file_uploaded_success', ['name' => '<span class="font-semibold" x-text="fileName"></span>']) !!}
                 </div>
             </template>
         </div>
