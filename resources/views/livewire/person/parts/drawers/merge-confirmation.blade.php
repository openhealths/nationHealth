...
>
    <x-slot name="title">
        {{ __('patients.merge.confirm_title', ['uuid' => strtoupper($uuid)]) }}
    </x-slot>
    ...
    <div class="mt-8 space-y-6">
        <div class="p-6 rounded-xl bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-700/50 flex gap-3.5">
            ...
            <div class="space-y-3.5 text-sm text-gray-755 dark:text-gray-300">
                <p class="font-bold text-gray-900 dark:text-white flex items-center">
                    {{ __('patients.merge.doctor_obligations') }}
                </p>
                <div class="space-y-1">
                    <p class="leading-relaxed">- {{ __('patients.merge.confirm_identified') }}</p>
                    <p class="leading-relaxed">- {{ __('patients.merge.confirm_selected_correctly') }}</p>
                    <p class="leading-relaxed">- {{ __('patients.merge.confirm_representative') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-6 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 flex flex-col gap-4">
            <div class="flex items-center gap-2">
                ...
                <h4 class="font-bold text-blue-800 dark:text-blue-300 text-sm tracking-wider">
                    {{ __('patients.merge.patient_memo') }}
                </h4>
            </div>
            <div class="space-y-3 text-blue-700 dark:text-blue-300 text-sm leading-relaxed">
                <p class="font-semibold">
                    {{ __('patients.merge.providing_code_or_docs') }}
                </p>
                <div class="space-y-1">
                    <p>- {{ __('patients.merge.memo_point_1') }}</p>
                    <p>- {{ __('patients.merge.memo_point_2') }}</p>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            >
                ...
                <span>{{ __('patients.merge.print_memo') }}</span>
            </button>
        </div>
        ...
        <div class="flex items-center gap-3 py-2">
            ...
            <label for="patientConsent" class="text-sm font-medium text-gray-900 dark:text-gray-100 select-none cursor-pointer">
                {{ __('patients.merge.memo_communicated') }}
            </label>
        </div>
    </div>
