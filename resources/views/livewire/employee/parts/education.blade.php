<div class="relative overflow-x-auto">
    <fieldset
        ...
        x-init="
            (() => {
                const initial = $wire.get('form.doctor.educations');
                educations = Array.isArray(initial) ? JSON.parse(JSON.stringify(initial)) : [];
            })();
            $watch('educations', (val) => {
                $wire.set('form.doctor.educations', Array.isArray(val) ? JSON.parse(JSON.stringify(val)) : [], false);
            });
        "
    >
        <legend class="legend">
            ...
            <template x-teleport="body">
                ...
                >
                    ...
                    <div x-show="openModal" x-transition @click="openModal = false" class="modal-wrapper">
                        ...
                        class="modal-content h-fit w-full max-w-6xl rounded-2xl bg-white shadow-lg"
                    >
                        <h3 class="modal-header" :id="$id('modal-title')">
                            <span
                                x-text="newEducation ? '{{ __('forms.addEducation') }}' : '{{ __('forms.edit') . ' ' . __('forms.education') }}'"
                            ></span>
                        </h3>
                        ...
                        <form>
                            <div class="form-row-modal">
                                <div>
                                    <label for="educationCountry" class="label-modal"
                                        >{{ __('forms.country') }}<span class="text-red-600"> *</span></label>
                                        <select
                                            x-model="modalEducation.country"
                                            id="educationCountry"
                                            class="input-modal"
                                            required
                                        >
                                    ...
                                    <div>
                                        <label for="educationCity" class="label-modal"
                                            >{{ __('forms.city') }}<span class="text-red-600"> *</span></label>
                                        <input
                                            x-model="modalEducation.city"
                                            type="text"
                                            id="educationCity"
                                            class="input-modal"
                                            required
                                    ...
                                    <div>
                                        <label for="educationInstitutionName" class="label-modal"
                                            >{{ __('forms.institutionName') }}
                                            <span class="text-red-600"> *</span></label>
                                        <input
                                            x-model="modalEducation.institutionName"
                                            type="text"
                                    ...
                                    <div>
                                        <label for="educationDegree" class="label-modal"
                                            >{{ __('forms.degree') }} <span class="text-red-600"> *</span></label>
                                        <select
                                        ...
                                        >
                                            ...
                                            <template x-if="employeeType && employeeTypeDegrees[employeeType]">
                                                <template
                                                    x-for="(degreeName, degreeKey) in employeeTypeDegrees[employeeType]"
                                                    :key="degreeKey"
                                                >
                                    ...
                                    <div class="relative">
                                        ...
                                        <label for="educationIssuedDate" class="label-modal"
                                            >{{ __('forms.issuedDate') }}<span class="text-red-600"> *</span></label>
                                        <input
                                            x-model="modalEducation.issuedDate"
                                            datepicker-format="{{ frontendDateFormat() }}"
                                            type="text"
                                            name="educationIssuedDate"
                                            id="educationIssuedDate"
                                            class="input-modal datepicker-input"
                                            autocomplete="off"
                                        />
                                    </div>
                                    <div>
                                        <label for="educationDiplomaNumber" class="label-modal"
                                            >{{ __('forms.diplomaNumber') }} <span class="text-red-600"> *</span></label>
                                        <input
                                            x-model="modalEducation.diplomaNumber"
                                            type="text"
                                            id="educationDiplomaNumber"
                                            class="input-modal"
                                        />
                                    </div>
                                </div>
                                ...
                                <div class="mt-6 flex flex-row items-center gap-4 border-t border-gray-200 pt-6">
                                    ...
                                    <button
                                        @click.prevent="
                                            newEducation
                                                ? educations.push({ ...modalEducation })
                                                : (educations[item] = { ...modalEducation });
                                            openModal = false;
                                            newEducation = null;
                                        "
